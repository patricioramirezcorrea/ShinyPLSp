...
# The segmentation variable is used only to partition observations. It is never
# sent to cSEM unless it is itself a manifest variable declared in the model.
# ==============================================================================
      ...
      bslib::card_body(
        bslib::layout_columns(
          ...
          shiny::conditionalPanel(
            ...
            shiny::numericInput(
              ns("segment_bins"), "Number of groups",
        ...
        shiny::tags$div(
          class = "small-muted",
          "The selected variable defines the comparison groups only; it is not estimated as part of the PLS-SEM model."
        ),
...
mod_micom_mga_server <- function(id, analysis_data_aug_rv, model_lavaan, result_rv,
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    get_sem_indicators <- function(model, data_names) {
      parsed <- tryCatch(cSEM::parseModel(model), error = function(e) NULL)
      if (is.null(parsed) || is.null(parsed$measurement)) {
        stop("The measurement model could not be parsed.")
      }

      indicators <- unique(unlist(parsed$measurement, use.names = FALSE))
      absent <- setdiff(indicators, data_names)
      if (length(absent)) {
        stop(paste0(
          "The following indicators specified in the model are absent from the data: ",
    ...
    shiny::observeEvent(input$btn_run_micom_mga, {
      ...
      analysis_out <- tryCatch({
        ...
        # Only model indicators are supplied to cSEM/MICOM/MGA.
        indicators <- get_sem_indicators(model, colnames(data_grouped))
        sem_data <- data_grouped[, indicators, drop = FALSE]

        non_numeric <- names(sem_data)[!vapply(sem_data, is.numeric, logical(1))]
        if (length(non_numeric)) {
          stop(paste0(
            "All manifest variables must be numeric for MICOM/MGA. Non-numeric indicator(s): ",
            paste(non_numeric, collapse = ", ")
          ))
        }

        if (anyNA(sem_data)) {
          stop("The analysis dataset contains missing values in SEM indicators after preprocessing.")
        }

        group_data <- split(sem_data, grouping, drop = TRUE)
